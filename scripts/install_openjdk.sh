...
then
  URL=`curl -s ${API_URL}/tags/${LATEST_JDK} | jq -r '.assets[] | select(.content_type == "application/x-compressed-tar") | select(.name | contains("jdk_x64_linux_hotspot")) | .browser_download_url'`
